...
from .ccomp cimport find_root, join_trees
...
def _felzenszwalb_grey(image, double scale=1, sigma=0.8,
    ...
    return  _felzenszwalb_graph(edges, costs, scale, min_size).reshape(height, width)

def _felzenszwalb_graph(cnp.ndarray[cnp.intp_t, ndim=2] edges, 
                        cnp.ndarray[cnp.float_t, ndim=1] costs,
                        double scale, 
                        Py_ssize_t min_size) :

    cdef cnp.ndarray[cnp.intp_t, ndim=1] segments = np.unique(edges)
    ...
    cdef cnp.ndarray[cnp.intp_t, ndim=1] segment_size \
            = np.ones_like(segments, dtype=np.intp)
    # inner cost of segments
    cdef cnp.ndarray[cnp.float_t, ndim=1] cint = np.zeros_like(segments, dtype=np.float)
    ...
    old = np.zeros_like(segments)
    while (old != segments).any():
        old = segments
        segments = segments[segments]
    segments = np.unique(segments, return_inverse=True)[1]
